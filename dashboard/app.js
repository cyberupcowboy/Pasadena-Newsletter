...
function getFilteredStories() {
  ...
  return stories.filter((story) => {
    const statusMatches = status === 'all'
      || (status === 'active' && ['review', 'new'].includes(story.editorial_status))
      || story.editorial_status === status;
...
async function saveStory(storyId, card, editorialStatus = null) {
  ...
  if (!headline || !summary) {
    saveState.textContent = 'Headline and summary are required.';
...
function renderStory(story) {
  ...
  card.querySelector('.category-badge').textContent = story.category || 'Uncategorized';

  const statusBadge = card.querySelector('.status-badge');
  statusBadge.textContent = statusLabel(story.editorial_status);
  statusBadge.dataset.status = story.editorial_status || 'unknown';
  ...
  const meta = [
    story.location_text,
    story.ai_processed_at ? `AI processed ${formatDate(story.ai_processed_at)}` : null,
    story.ai_model ? `Model: ${story.ai_model}` : null,
  ].filter(Boolean).join(' · ');
  card.querySelector('.story-meta').textContent = meta;

  const relevance = clampScore(story.pasadena_relevance);
  const urgency = clampScore(story.urgency);
  card.querySelector('.relevance-score strong').textContent = relevance;
  card.querySelector('.urgency-score strong').textContent = urgency;
  card.querySelector('.relevance-score').style.setProperty('--score', relevance);
  card.querySelector('.urgency-score').style.setProperty('--score', urgency);
  ...
  if (story.editorial_status === 'approved') {
    card.querySelector('.approve-button').disabled = true;
  }
  if (story.editorial_status === 'rejected') {
  ...
  if (!filtered.length) {
    setNotice(
      stories.length
        ...
        : 'No accessible stories were returned. If this is a new login, the account may not yet be on the editor list.',
      'info',
    );
    return;
  }
...
async function loadStories({ quiet = false } = {}) {
  ...
  if (error) {
    stories = [];
    setNotice(`Could not load the queue: ${error.message}`, 'error');
    updateStats();
    renderStories();
    return;
  }
...
function renderSession(session) {
  ...
  setHidden(els.loginPanel, signedIn);
  setHidden(els.dashboard, !signedIn);
  setHidden(els.signOutButton, !signedIn);
  setHidden(els.sessionEmail, !signedIn);
...
els.loginForm.addEventListener('submit', async (event) => {
  ...
  els.loginMessage.dataset.kind = 'info';
  ...
  if (error) {
    ...
    els.loginMessage.dataset.kind = 'error';
    return;
  }
...
supabase.auth.onAuthStateChange((_event, session) => {
  if (session?.access_token !== currentSession?.access_token) renderSession(session);
});
